...
# === CONFIG ===
CSV_FILE = "message_phishing_detection.csv"
...
# === Sanity Check ===
if "message_body" not in df.columns or "is_phishing" not in df.columns:
    raise ValueError("CSV must contain 'message_body' and 'is_phishing' columns.")

# === Check Class Balance ===
print("Class distribution before training:")
print(df["is_phishing"].value_counts())

# === STEP 2: Add Dummy Phishing Data If Needed ===
if df["is_phishing"].value_counts().get(1, 0) == 0:
...
# === STEP 1: Dummy Data Injection (Optional) ===
dummy_texts = [
    "Urgent! Update your password immediately.",
    "You've won a prize! Click here to claim.",
    "Unusual login detected. Verify your identity."
]
dummy_rows = pd.DataFrame({
    "message_body": dummy_texts,
    "is_phishing": [1] * len(dummy_texts)
})
...
# === STEP 3: Clean Text and Define Features ===
df["cleaned_text"] = df["message_body"].apply(preprocess_text)
X = df["cleaned_text"]
...
MODEL_FILE = "logistic_phishing_model.pkl"
VECTORIZER_FILE = "tfidf_vectorizer.pkl"
...
print(f"\n🎯 Final Accuracy Score: {final_score * 100:.2f}%")

# === OPTIONAL: Test the Predictor ===
test_msg = "immediate action required, validate your account."
print(f"\n🔍 Test Message: '{test_msg}'")
print("🧠 Prediction:", predict_phishing(test_msg))
